<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Dengue Surveillance Dashboard - Bangladesh 2026</title>

    <!-- Bootstrap 5 CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" />
    <!-- Font Awesome 6 -->
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet" />
    <!-- DataTables Bootstrap 5 -->
    <link href="https://cdn.datatables.net/1.13.6/css/dataTables.bootstrap5.min.css" rel="stylesheet" />

    <style>
        :root {
            --primary-bg: #0a3d62;
            --secondary-bg: #1e7e34;
            --accent: #e74c3c;
            --card-shadow: 0 8px 24px rgba(0, 0, 0, 0.12);
            --border-radius: 16px;
            --transition: 0.3s ease;
        }

        * {
            box-sizing: border-box;
        }

        body {
            background: #f0f4f8;
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            padding: 1.5rem;
            color: #1a1a2e;
        }

        .dashboard-wrapper {
            max-width: 1440px;
            margin: 0 auto;
        }

        /* ── Header ── */
        .header-card {
            background: linear-gradient(135deg, var(--primary-bg) 0%, #1a5276 100%);
            color: #fff;
            border-radius: var(--border-radius);
            padding: 2rem 2.5rem;
            margin-bottom: 2rem;
            box-shadow: var(--card-shadow);
            position: relative;
            overflow: hidden;
        }
        .header-card::after {
            content: '';
            position: absolute;
            top: -50%;
            right: -10%;
            width: 400px;
            height: 400px;
            background: rgba(255, 255, 255, 0.04);
            border-radius: 50%;
            pointer-events: none;
        }
        .header-card .logo-area {
            display: flex;
            align-items: center;
            gap: 1rem;
            flex-wrap: wrap;
        }
        .header-card .logo-area i {
            font-size: 2.8rem;
            opacity: 0.9;
        }
        .header-card .title-main {
            font-size: 1.8rem;
            font-weight: 700;
            letter-spacing: 0.5px;
            margin: 0;
        }
        .header-card .title-sub {
            font-size: 1rem;
            opacity: 0.85;
            font-weight: 400;
            margin: 0.2rem 0 0 0;
        }
        .header-card .date-badge {
            background: rgba(255, 255, 255, 0.18);
            backdrop-filter: blur(4px);
            padding: 0.4rem 1.4rem;
            border-radius: 40px;
            font-size: 0.95rem;
            font-weight: 500;
            display: inline-flex;
            align-items: center;
            gap: 0.5rem;
            margin-left: auto;
        }
        .header-card .date-badge i {
            font-size: 1rem;
        }

        @media (max-width: 768px) {
            .header-card {
                padding: 1.5rem;
            }
            .header-card .title-main {
                font-size: 1.3rem;
            }
            .header-card .date-badge {
                margin-left: 0;
                margin-top: 0.5rem;
                width: 100%;
                justify-content: center;
            }
            .header-card .logo-area {
                flex-direction: column;
                align-items: flex-start;
            }
        }

        /* ── Stats Cards ── */
        .stat-card {
            background: #fff;
            border-radius: var(--border-radius);
            padding: 1.25rem 1.5rem;
            box-shadow: var(--card-shadow);
            transition: transform var(--transition), box-shadow var(--transition);
            height: 100%;
            border-left: 5px solid var(--primary-bg);
            position: relative;
            overflow: hidden;
        }
        .stat-card:hover {
            transform: translateY(-4px);
            box-shadow: 0 12px 32px rgba(0, 0, 0, 0.15);
        }
        .stat-card .stat-icon {
            font-size: 2.2rem;
            opacity: 0.2;
            position: absolute;
            right: 1rem;
            bottom: 0.8rem;
        }
        .stat-card .stat-label {
            font-size: 0.85rem;
            text-transform: uppercase;
            letter-spacing: 0.5px;
            color: #6c757d;
            font-weight: 600;
            margin-bottom: 0.25rem;
        }
        .stat-card .stat-number {
            font-size: 2.2rem;
            font-weight: 700;
            color: var(--primary-bg);
            line-height: 1.2;
        }
        .stat-card .stat-sub {
            font-size: 0.85rem;
            color: #6c757d;
            margin-top: 0.2rem;
        }
        .stat-card.accent {
            border-left-color: var(--accent);
        }
        .stat-card.accent .stat-number {
            color: var(--accent);
        }
        .stat-card.success {
            border-left-color: var(--secondary-bg);
        }
        .stat-card.success .stat-number {
            color: var(--secondary-bg);
        }
        .stat-card.warning {
            border-left-color: #f39c12;
        }
        .stat-card.warning .stat-number {
            color: #f39c12;
        }
        .stat-card.info {
            border-left-color: #3498db;
        }
        .stat-card.info .stat-number {
            color: #3498db;
        }

        @media (max-width: 576px) {
            .stat-card .stat-number {
                font-size: 1.6rem;
            }
            .stat-card {
                padding: 1rem 1.2rem;
            }
        }

        /* ── Section Cards ── */
        .section-card {
            background: #fff;
            border-radius: var(--border-radius);
            box-shadow: var(--card-shadow);
            padding: 1.5rem 1.8rem;
            margin-bottom: 1.8rem;
            transition: box-shadow var(--transition);
        }
        .section-card:hover {
            box-shadow: 0 12px 32px rgba(0, 0, 0, 0.10);
        }
        .section-card .section-title {
            font-size: 1.3rem;
            font-weight: 700;
            color: var(--primary-bg);
            margin-bottom: 1.2rem;
            display: flex;
            align-items: center;
            gap: 0.6rem;
        }
        .section-card .section-title i {
            color: var(--accent);
            font-size: 1.2rem;
        }

        /* ── Tables ── */
        .table-responsive-custom {
            overflow-x: auto;
        }
        .table thead th {
            background: var(--primary-bg);
            color: #fff;
            font-weight: 600;
            font-size: 0.85rem;
            text-transform: uppercase;
            letter-spacing: 0.3px;
            padding: 0.75rem 0.8rem;
            border: none;
        }
        .table tbody td {
            padding: 0.65rem 0.8rem;
            vertical-align: middle;
            font-size: 0.92rem;
            border-bottom: 1px solid #e9ecef;
        }
        .table tbody tr:hover {
            background: #f8f9fa;
        }
        .table .total-row {
            background: #eef2f7 !important;
            font-weight: 700;
        }
        .table .total-row td {
            border-top: 2px solid var(--primary-bg);
        }
        .badge-dhaka {
            background: #2c3e50;
            color: #fff;
        }
        .badge-other {
            background: #e9ecef;
            color: #495057;
        }

        .dataTables_wrapper .dataTables_filter input {
            border: 1px solid #ced4da;
            border-radius: 30px;
            padding: 0.3rem 1rem;
            outline: none;
        }
        .dataTables_wrapper .dataTables_filter input:focus {
            border-color: var(--primary-bg);
            box-shadow: 0 0 0 3px rgba(10, 61, 98, 0.15);
        }
        .dataTables_wrapper .dataTables_length select {
            border: 1px solid #ced4da;
            border-radius: 30px;
            padding: 0.2rem 0.8rem;
        }

        /* ── Charts ── */
        .chart-container {
            position: relative;
            height: 280px;
            width: 100%;
        }
        .chart-container-sm {
            height: 220px;
        }
        .chart-container-lg {
            height: 320px;
        }

        .chart-grid {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1.5rem;
        }

        @media (max-width: 992px) {
            .chart-grid {
                grid-template-columns: 1fr;
                gap: 1.8rem;
            }
        }

        /* ── Footer ── */
        .footer-note {
            text-align: center;
            color: #6c757d;
            font-size: 0.85rem;
            padding: 1.5rem 0 0.5rem 0;
            border-top: 1px solid #e9ecef;
            margin-top: 1.5rem;
        }
        .footer-note i {
            color: var(--accent);
        }

        /* ── Misc ── */
        .text-muted-sm {
            font-size: 0.8rem;
            color: #6c757d;
        }
        .badge-gov {
            background: #2e86de;
            color: #fff;
            font-size: 0.7rem;
            padding: 0.25rem 0.6rem;
            border-radius: 20px;
        }
        .badge-pvt {
            background: #f39c12;
            color: #fff;
            font-size: 0.7rem;
            padding: 0.25rem 0.6rem;
            border-radius: 20px;
        }

        /* scrollable table wrapper */
        .table-scroll {
            max-height: 420px;
            overflow-y: auto;
        }
        .table-scroll thead th {
            position: sticky;
            top: 0;
            z-index: 5;
        }

        /* responsive tweaks */
        @media (max-width: 576px) {
            .section-card {
                padding: 1rem;
            }
            .section-card .section-title {
                font-size: 1.1rem;
            }
            .header-card .title-main {
                font-size: 1.1rem;
            }
            body {
                padding: 0.75rem;
            }
        }
    </style>
</head>
<body>

    <div class="dashboard-wrapper">

        <!-- ===== HEADER ===== -->
        <div class="header-card">
            <div class="d-flex flex-wrap align-items-center justify-content-between">
                <div class="logo-area">
                    <i class="fas fa-shield-virus"></i>
                    <div>
                        <div class="title-main">গণপ্রজাতন্ত্রী বাংলাদেশ সরকার</div>
                        <div class="title-sub">স্বাস্থ্য ও পরিবার কল্যাণ মন্ত্রণালয় · স্বাস্থ্য সেবা বিভাগ</div>
                        <div class="title-sub" style="font-size:0.85rem;opacity:0.75;">
                            হেলথ ইমার্জেন্সি অপারেশন সেন্টার ও কন্ট্রোল রুম, মহাখালী, ঢাকা
                        </div>
                    </div>
                </div>
                <div class="date-badge">
                    <i class="fas fa-calendar-day"></i>
                    <span>আপডেট: ১০ আগস্ট ২০২৬</span>
                </div>
            </div>
        </div>

        <!-- ===== STATS ROW ===== -->
        <div class="row g-3 mb-4">
            <div class="col-6 col-md-4 col-lg-2">
                <div class="stat-card">
                    <div class="stat-label"><i class="fas fa-plus-circle me-1"></i>২৪ ঘণ্টায় নতুন</div>
                    <div class="stat-number">৭২৬</div>
                    <div class="stat-sub">আক্রান্ত</div>
                    <i class="fas fa-user-plus stat-icon"></i>
                </div>
            </div>
            <div class="col-6 col-md-4 col-lg-2">
                <div class="stat-card info">
                    <div class="stat-label"><i class="fas fa-virus me-1"></i>মোট আক্রান্ত</div>
                    <div class="stat-number">২০,০৭১</div>
                    <div class="stat-sub">০১ জানুয়ারি হতে</div>
                    <i class="fas fa-chart-line stat-icon"></i>
                </div>
            </div>
            <div class="col-6 col-md-4 col-lg-2">
                <div class="stat-card accent">
                    <div class="stat-label"><i class="fas fa-skull me-1"></i>মোট মৃত্যু</div>
                    <div class="stat-number">৬১</div>
                    <div class="stat-sub">সর্বমোট</div>
                    <i class="fas fa-heartbeat stat-icon"></i>
                </div>
            </div>
            <div class="col-6 col-md-4 col-lg-2">
                <div class="stat-card success">
                    <div class="stat-label"><i class="fas fa-check-circle me-1"></i>ছাড়পত্র প্রাপ্ত</div>
                    <div class="stat-number">১,৮৫৬</div>
                    <div class="stat-sub">সুস্থ হয়েছেন</div>
                    <i class="fas fa-home stat-icon"></i>
                </div>
            </div>
            <div class="col-6 col-md-4 col-lg-2">
                <div class="stat-card warning">
                    <div class="stat-label"><i class="fas fa-hospital-user me-1"></i>বর্তমানে ভর্তি</div>
                    <div class="stat-number">১,৪৪৩</div>
                    <div class="stat-sub">হাসপাতালে</div>
                    <i class="fas fa-bed stat-icon"></i>
                </div>
            </div>
            <div class="col-6 col-md-4 col-lg-2">
                <div class="stat-card" style="border-left-color:#8e44ad;">
                    <div class="stat-label"><i class="fas fa-venus-mars me-1"></i>লিঙ্গ অনুপাত</div>
                    <div class="stat-number" style="font-size:1.6rem;color:#8e44ad;">৬১.৬% / ৩৮.৪%</div>
                    <div class="stat-sub">পুরুষ / নারী</div>
                    <i class="fas fa-people-arrows stat-icon"></i>
                </div>
            </div>
        </div>

        <!-- ===== DIVISION SUMMARY TABLE ===== -->
        <div class="section-card">
            <div class="section-title">
                <i class="fas fa-map-marked-alt"></i> বিভাগভিত্তিক সংক্ষিপ্ত তথ্য
                <span class="ms-auto text-muted-sm" style="font-weight:400;font-size:0.8rem;">
                    <i class="far fa-calendar-alt"></i> ০১ জানুয়ারি – ১০ আগস্ট ২০২৬
                </span>
            </div>
            <div class="table-responsive-custom">
                <table class="table table-bordered table-hover" id="divisionTable">
                    <thead>
                        <tr>
                            <th>ক্রমিক</th>
                            <th>বিভাগ</th>
                            <th>২৪ ঘণ্টায় নতুন</th>
                            <th>মোট আক্রান্ত</th>
                            <th>বর্তমানে ভর্তি</th>
                            <th>মৃত্যু</th>
                            <th>ছাড়পত্র প্রাপ্ত</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr><td>১</td><td><strong>ঢাকা বিভাগ</strong></td><td>৩৪৬</td><td>৭,৮৪৫</td><td>৫২৮</td><td>৭২৭</td><td>৬,৫৪১</td></tr>
                        <tr><td>২</td><td>ময়মনসিংহ বিভাগ</td><td>৩৭</td><td>৮৮</td><td>৫৬</td><td>৭</td><td>৬১</td></tr>
                        <tr><td>৩</td><td>চট্টগ্রাম বিভাগ</td><td>৮১</td><td>৩৩৯</td><td>৭৫</td><td>৩২</td><td>২৮৫</td></tr>
                        <tr><td>৪</td><td>খুলনা বিভাগ</td><td>১০৭</td><td>২৬৪</td><td>৯১</td><td>২২</td><td>৩৩৯</td></tr>
                        <tr><td>৫</td><td>রাজশাহী বিভাগ</td><td>৬৫</td><td>৮৩</td><td>৫১</td><td>৭</td><td>৬১</td></tr>
                        <tr><td>৬</td><td>রংপুর বিভাগ</td><td>১০০</td><td>৪৪১</td><td>১৪</td><td>১৭</td><td>২৩৭</td></tr>
                        <tr><td>৭</td><td>বরিশাল বিভাগ</td><td>৭৫</td><td>৩৮৯</td><td>৯৭</td><td>৩৬</td><td>১৪২</td></tr>
                        <tr><td>৮</td><td>সিলেট বিভাগ</td><td>৫০</td><td>১২০</td><td>১১</td><td>১৪</td><td>১৪৫</td></tr>
                    </tbody>
                    <tfoot>
                        <tr class="total-row">
                            <td colspan="2" style="text-align:right;font-weight:700;">সর্বমোট</td>
                            <td><strong>৭২৬</strong></td>
                            <td><strong>২০,০৭১</strong></td>
                            <td><strong>১,৪৪৩</strong></td>
                            <td><strong>৬১</strong></td>
                            <td><strong>১,৮৫৬</strong></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            <div class="text-muted-sm mt-2">
                <i class="fas fa-info-circle"></i> তথ্য সুত্র: স্বাস্থ্য অধিদপ্তর, এমআইএস, হেলথ ইমার্জেন্সি অপারেশন সেন্টার ও কন্ট্রোল রুম
            </div>
        </div>

        <!-- ===== CHARTS SECTION ===== -->
        <div class="section-card">
            <div class="section-title">
                <i class="fas fa-chart-pie"></i> পরিসংখ্যানিক বিশ্লেষণ
                <span class="ms-auto text-muted-sm" style="font-weight:400;font-size:0.8rem;">
                    <i class="far fa-calendar-alt"></i> ০১ জানুয়ারি – ১০ আগস্ট ২০২৬
                </span>
            </div>

            <div class="chart-grid">

                <!-- Gender – Cases -->
                <div>
                    <h6 class="text-center fw-semibold mb-2" style="color:var(--primary-bg);">
                        <i class="fas fa-venus-mars me-1"></i> আক্রান্তের লিঙ্গ বণ্টন
                    </h6>
                    <div class="chart-container">
                        <canvas id="genderCasesChart"></canvas>
                    </div>
                    <div class="text-center text-muted-sm mt-1">
                        পুরুষ ১২,৩৬৪ · নারী ৭,৭০২
                    </div>
                </div>

                <!-- Gender – Deaths -->
                <div>
                    <h6 class="text-center fw-semibold mb-2" style="color:var(--accent);">
                        <i class="fas fa-venus-mars me-1"></i> মৃত্যুর লিঙ্গ বণ্টন
                    </h6>
                    <div class="chart-container">
                        <canvas id="genderDeathsChart"></canvas>
                    </div>
                    <div class="text-center text-muted-sm mt-1">
                        পুরুষ ২২ · নারী ৩৯
                    </div>
                </div>

                <!-- Age – Cases -->
                <div>
                    <h6 class="text-center fw-semibold mb-2" style="color:var(--primary-bg);">
                        <i class="fas fa-users me-1"></i> বয়সভিত্তিক আক্রান্ত (২০২৬)
                    </h6>
                    <div class="chart-container chart-container-lg">
                        <canvas id="ageCasesChart"></canvas>
                    </div>
                </div>

                <!-- Age – Deaths -->
                <div>
                    <h6 class="text-center fw-semibold mb-2" style="color:var(--accent);">
                        <i class="fas fa-user-injured me-1"></i> বয়সভিত্তিক মৃত্যু (২০২৬)
                    </h6>
                    <div class="chart-container chart-container-lg">
                        <canvas id="ageDeathsChart"></canvas>
                    </div>
                </div>

                <!-- Monthly Deaths -->
                <div style="grid-column: 1 / -1;">
                    <h6 class="text-center fw-semibold mb-2" style="color:var(--accent);">
                        <i class="fas fa-calendar-check me-1"></i> মাসভিত্তিক নিশ্চিত মৃত্যু
                    </h6>
                    <div class="chart-container chart-container-lg">
                        <canvas id="monthlyDeathsChart"></canvas>
                    </div>
                </div>

            </div>
        </div>

        <!-- ===== HOSPITAL-WISE TABLE ===== -->
        <div class="section-card">
            <div class="section-title">
                <i class="fas fa-hospital"></i> হাসপাতালভিত্তিক ডেঙ্গু তথ্য
                <span class="ms-auto text-muted-sm" style="font-weight:400;font-size:0.8rem;">
                    <i class="far fa-calendar-alt"></i> ০১ জানুয়ারি – ১০ আগস্ট ২০২৬
                </span>
            </div>

            <div class="table-responsive-custom">
                <table class="table table-bordered table-hover" id="hospitalTable">
                    <thead>
                        <tr>
                            <th>ক্রমিক</th>
                            <th>হাসপাতালের নাম</th>
                            <th>২৪ ঘণ্টায় নতুন</th>
                            <th>মোট আক্রান্ত</th>
                            <th>বর্তমানে ভর্তি</th>
                            <th>মৃত্যু</th>
                            <th>ছাড়পত্র প্রাপ্ত</th>
                        </tr>
                    </thead>
                    <tbody>
                        <!-- Data from PDF page 3 & 6 (non-zero rows) -->
                        <tr><td>৪</td><td>আজগি আলী হাসপাতাল</td><td>০</td><td>৬৩</td><td>০</td><td>৬১</td><td>২</td></tr>
                        <tr><td>২৬</td><td>বাংলামেশ রেশালাইজ্ড হাসপাতাল</td><td>০</td><td>৩০</td><td>০</td><td>২৫</td><td>৫</td></tr>
                        <tr><td>২৮</td><td>সালাউরিন হাসপাতাল</td><td>৪</td><td>৪২</td><td>০</td><td>৪০</td><td>২</td></tr>
                        <tr><td>২৯</td><td>পপুলার মেডিকেল কলেজ হাসপাতাল</td><td>০</td><td>৪৩</td><td>০</td><td>৪১</td><td>২</td></tr>
                        <tr><td>৩৩</td><td>ডা: এ জি খান শিশু হাসপাতাল</td><td>৩</td><td>৬৯</td><td>০</td><td>৬২</td><td>৭</td></tr>
                        <tr><td>৩৯</td><td>এ এ রজি হাসপাতাল</td><td>০</td><td>১৩</td><td>০</td><td>১২</td><td>১</td></tr>
                        <tr><td>৪৩</td><td>এরি ব্যাংক হাসপাতাল</td><td>০</td><td>২৩</td><td>০</td><td>১৮</td><td>৫</td></tr>
                        <tr><td>৫২</td><td>আমলাক হাসপাতাল (টুং)</td><td>২</td><td>৫৩</td><td>০</td><td>৪৮</td><td>৫</td></tr>
                        <tr><td>৫৯</td><td>ইনসা বাংলা কিরন এন্ড জেনারেল হাসপাতাল</td><td>০</td><td>৭</td><td>০</td><td>৪</td><td>৩</td></tr>
                        <tr><td>৬০</td><td>লাল মনিহাট</td><td>০</td><td>৫৯</td><td>০</td><td>৫৭</td><td>২</td></tr>
                        <tr><td>৬১</td><td>কুড়িগ্রাম</td><td>০</td><td>৩০</td><td>০</td><td>২৮</td><td>২</td></tr>
                        <tr><td>৬২</td><td>নীলফামারী</td><td>০</td><td>৭৯</td><td>০</td><td>৭৮</td><td>১</td></tr>
                        <tr><td>৬৩</td><td>দিনাজপুর</td><td>০</td><td>৪৪</td><td>০</td><td>৪২</td><td>২</td></tr>
                        <tr><td>৬৪</td><td>গাইবান্ধা</td><td>১</td><td>৪৩</td><td>০</td><td>৩৯</td><td>৪</td></tr>
                        <tr><td>৬৫</td><td>ঠাকুরগাঁও</td><td>৩</td><td>২০</td><td>০</td><td>১৯</td><td>১</td></tr>
                        <tr><td>৬৬</td><td>পঞ্চগড়</td><td>০</td><td>৫</td><td>০</td><td>৫</td><td>০</td></tr>
                        <tr><td>৬৯</td><td>বরিশাল</td><td>১০</td><td>৫৩৯</td><td>০</td><td>৫১০</td><td>২৯</td></tr>
                        <tr><td>৭০</td><td>পটুয়াখালী</td><td>৩</td><td>৪০৫</td><td>০</td><td>৩৯৫</td><td>১০</td></tr>
                        <tr><td>৭২</td><td>ভোলা</td><td>৭</td><td>১৭৯</td><td>০</td><td>১৬৫</td><td>১৪</td></tr>
                        <tr><td>৭৩</td><td>পিরোজপুর</td><td>২৭</td><td>১০৪০</td><td>১</td><td>৯৭৭</td><td>৬২</td></tr>
                        <tr><td>৭৪</td><td>ঝিগুনা</td><td>১১</td><td>৪৩৮</td><td>০</td><td>৩৯৬</td><td>৪২</td></tr>
                        <tr><td>৭৫</td><td>ঝালকাঠি</td><td>১৫</td><td>৯২৫</td><td>১</td><td>৮৮৪</td><td>৪০</td></tr>
                        <tr><td>৭৮</td><td>সুনামগঞ্জ</td><td>৩</td><td>১৭</td><td>০</td><td>১৪</td><td>৩</td></tr>
                        <tr><td>৭৯</td><td>হবিগঞ্জ</td><td>০</td><td>৪৬</td><td>০</td><td>৪৪</td><td>২</td></tr>
                        <tr><td>৮০</td><td>ব্রাহ্মণবাড়িয়া</td><td>২</td><td>৫</td><td>০</td><td>৫</td><td>০</td></tr>
                    </tbody>
                    <tfoot>
                        <tr class="total-row">
                            <td colspan="2" style="text-align:right;font-weight:700;">সর্বমোট (উপরে তালিকাভুক্ত)</td>
                            <td><strong>৮২</strong></td>
                            <td><strong>৪,২৬২</strong></td>
                            <td><strong>২</strong></td>
                            <td><strong>৩,৯৪২</strong></td>
                            <td><strong>৩১৮</strong></td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            <div class="text-muted-sm mt-2">
                <i class="fas fa-info-circle"></i> শুধুমাত্র অ-শূন্য ডেটা সহ হাসপাতালসমূহ দেখানো হয়েছে। সম্পূর্ণ তালিকার জন্য মূল প্রতিবেদন দ্রষ্টব্য।
            </div>
        </div>

        <!-- ===== FOOTER ===== -->
        <div class="footer-note">
            <i class="fas fa-database me-1"></i> তথ্য উৎস: স্বাস্থ্য অধিদপ্তর, এমআইএস, হেলথ ইমার্জেন্সি অপারেশন সেন্টার ও কন্ট্রোল রুম, মহাখালী, ঢাকা
            <br>
            <span style="font-size:0.75rem;">
                হটলাইন: ০১৭৫৯-১১৪৪৮৮ · টেলিফোন: ০২-২২২২৮৫৯৩৩, ০২-২২২২৯৭৩৫৩ · ই-মেইল: controlroomdghs@yahoo.com
            </span>
            <br>
            <span style="font-size:0.7rem;opacity:0.7;">
                সর্বশেষ আপডেট: ১০ আগস্ট ২০২৬
            </span>
        </div>

    </div>

    <!-- ===== SCRIPTS ===== -->
    <script src="https://code.jquery.com/jquery-3.7.0.min.js">
    </script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js">
    </script>
    <script src="https://cdn.datatables.net/1.13.6/js/jquery.dataTables.min.js">
    </script>
    <script src="https://cdn.datatables.net/1.13.6/js/dataTables.bootstrap5.min.js">
    </script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js">
    </script>

    <script>
        $(document).ready(function() {

            // ---- DataTables ----
            $('#divisionTable').DataTable({
                responsive: true,
                paging: false,
                ordering: true,
                info: false,
                searching: true,
                columnDefs: [
                    { targets: [0, 1], orderable: true },
                    { targets: '_all', orderable: false }
                ],
                language: {
                    search: "খুঁজুন: ",
                    zeroRecords: "কোনো তথ্য পাওয়া যায়নি",
                }
            });

            $('#hospitalTable').DataTable({
                responsive: true,
                paging: true,
                pageLength: 10,
                lengthMenu: [
                    [5, 10, 25, 50, -1],
                    [5, 10, 25, 50, "সব"]
                ],
                order: [
                    [3, 'desc']
                ],
                columnDefs: [
                    { targets: [0, 1], orderable: true },
                    { targets: '_all', orderable: false }
                ],
                language: {
                    search: "খুঁজুন: ",
                    lengthMenu: "প্রতি পৃষ্ঠায় _MENU_ টি",
                    info: "_START_–_END_ / _TOTAL_",
                    infoEmpty: "০–০ / ০",
                    zeroRecords: "কোনো তথ্য পাওয়া যায়নি",
                    paginate: {
                        first: "প্রথম",
                        last: "শেষ",
                        next: "পরবর্তী",
                        previous: "পূর্ববর্তী"
                    }
                }
            });

            // ---- Chart Colors ----
            const colors = {
                blue: '#2c3e50',
                red: '#e74c3c',
                green: '#1e7e34',
                orange: '#f39c12',
                purple: '#8e44ad',
                teal: '#1abc9c',
                pink: '#e84393',
                grey: '#95a5a6',
                darkBlue: '#0a3d62',
                lightBlue: '#3498db'
            };

            const palette = ['#0a3d62', '#1e7e34', '#e74c3c', '#f39c12', '#8e44ad', '#1abc9c', '#e84393', '#3498db', '#2c3e50',
                '#d35400'
            ];

            // ---- 1. Gender Cases (Pie) ----
            const ctx1 = document.getElementById('genderCasesChart').getContext('2d');
            new Chart(ctx1, {
                type: 'doughnut',
                data: {
                    labels: ['পুরুষ', 'নারী'],
                    datasets: [{
                        data: [12364, 7702],
                        backgroundColor: ['#2c3e50', '#e74c3c'],
                        borderWidth: 2,
                        borderColor: '#fff'
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: {
                            position: 'bottom',
                            labels: {
                                font: { size: 13, weight: '500' },
                                padding: 16
                            }
                        },
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    let total = ctx.dataset.data.reduce((a, b) => a + b, 0);
                                    let pct = ((ctx.parsed / total) * 100).toFixed(1);
                                    return ctx.label + ': ' + ctx.parsed.toLocaleString() + ' (' + pct + '%)';
                                }
                            }
                        }
                    }
                }
            });

            // ---- 2. Gender Deaths (Pie) ----
            const ctx2 = document.getElementById('genderDeathsChart').getContext('2d');
            new Chart(ctx2, {
                type: 'doughnut',
                data: {
                    labels: ['পুরুষ', 'নারী'],
                    datasets: [{
                        data: [22, 39],
                        backgroundColor: ['#2c3e50', '#e74c3c'],
                        borderWidth: 2,
                        borderColor: '#fff'
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: {
                            position: 'bottom',
                            labels: {
                                font: { size: 13, weight: '500' },
                                padding: 16
                            }
                        },
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    let total = ctx.dataset.data.reduce((a, b) => a + b, 0);
                                    let pct = ((ctx.parsed / total) * 100).toFixed(1);
                                    return ctx.label + ': ' + ctx.parsed + ' (' + pct + '%)';
                                }
                            }
                        }
                    }
                }
            });

            // ---- 3. Age Cases (Bar) ----
            const ageGroups = ['0-5', '6-10', '11-15', '16-20', '21-25', '26-30', '31-35', '36-40', '41-45', '46-50', '51-55',
                '56-60', '61-65', '66-70', '71-75', '76-80', '80+'
            ];
            const ageCasesMale = [590, 583, 826, 1748, 1800, 1844, 1178, 1042, 762, 619, 415, 389, 236, 170, 66, 56, 38];
            const ageCasesFemale = [472, 477, 512, 907, 950, 955, 810, 744, 534, 476, 279, 266, 159, 91, 35, 16, 17];

            const ctx3 = document.getElementById('ageCasesChart').getContext('2d');
            new Chart(ctx3, {
                type: 'bar',
                data: {
                    labels: ageGroups,
                    datasets: [{
                        label: 'পুরুষ',
                        data: ageCasesMale,
                        backgroundColor: 'rgba(44,62,80,0.75)',
                        borderColor: '#2c3e50',
                        borderWidth: 1
                    }, {
                        label: 'নারী',
                        data: ageCasesFemale,
                        backgroundColor: 'rgba(231,76,60,0.7)',
                        borderColor: '#e74c3c',
                        borderWidth: 1
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: {
                            position: 'top',
                            labels: {
                                font: { size: 12, weight: '500' },
                                usePointStyle: true,
                                pointStyle: 'circle'
                            }
                        },
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    return ctx.dataset.label + ': ' + ctx.parsed.y.toLocaleString();
                                }
                            }
                        }
                    },
                    scales: {
                        x: {
                            grid: { display: false },
                            ticks: { font: { size: 10 } }
                        },
                        y: {
                            beginAtZero: true,
                            grid: { color: 'rgba(0,0,0,0.05)' },
                            ticks: { font: { size: 11 } }
                        }
                    }
                }
            });

            // ---- 4. Age Deaths (Bar) ----
            const ageDeaths = [12, 1, 2, 1, 3, 10, 6, 4, 0, 5, 2, 0, 3, 0, 2, 0, 0];
            const ctx4 = document.getElementById('ageDeathsChart').getContext('2d');
            new Chart(ctx4, {
                type: 'bar',
                data: {
                    labels: ageGroups,
                    datasets: [{
                        label: 'মৃত্যু',
                        data: ageDeaths,
                        backgroundColor: 'rgba(231,76,60,0.75)',
                        borderColor: '#e74c3c',
                        borderWidth: 1,
                        borderRadius: 4
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: {
                            display: false
                        },
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    return 'মৃত্যু: ' + ctx.parsed.y;
                                }
                            }
                        }
                    },
                    scales: {
                        x: {
                            grid: { display: false },
                            ticks: { font: { size: 10 } }
                        },
                        y: {
                            beginAtZero: true,
                            grid: { color: 'rgba(0,0,0,0.05)' },
                            ticks: {
                                stepSize: 2,
                                font: { size: 11 }
                            }
                        }
                    }
                }
            });

            // ---- 5. Monthly Deaths (Bar) ----
            const months = ['জানুয়ারি', 'ফেব্রুয়ারি', 'মার্চ', 'এপ্রিল', 'মে', 'জুন', 'জুলাই', 'আগস্ট'];
            const monthlyDeaths = [1, 0, 0, 0, 2, 3, 14, 41]; // from PDF page 11

            const ctx5 = document.getElementById('monthlyDeathsChart').getContext('2d');
            new Chart(ctx5, {
                type: 'bar',
                data: {
                    labels: months,
                    datasets: [{
                        label: 'নিশ্চিত মৃত্যু',
                        data: monthlyDeaths,
                        backgroundColor: [
                            'rgba(44,62,80,0.6)', 'rgba(149,165,166,0.5)',
                            'rgba(149,165,166,0.5)', 'rgba(149,165,166,0.5)',
                            'rgba(243,156,18,0.7)', 'rgba(231,76,60,0.7)',
                            'rgba(231,76,60,0.85)', 'rgba(192,57,43,0.95)'
                        ],
                        borderColor: [
                            '#2c3e50', '#95a5a6', '#95a5a6', '#95a5a6',
                            '#f39c12', '#e74c3c', '#e74c3c', '#c0392b'
                        ],
                        borderWidth: 1,
                        borderRadius: 4
                    }]
                },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    plugins: {
                        legend: {
                            display: false
                        },
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    return 'মৃত্যু: ' + ctx.parsed.y;
                                }
                            }
                        }
                    },
                    scales: {
                        x: {
                            grid: { display: false },
                            ticks: { font: { size: 11, weight: '500' } }
                        },
                        y: {
                            beginAtZero: true,
                            grid: { color: 'rgba(0,0,0,0.06)' },
                            ticks: {
                                stepSize: 5,
                                font: { size: 11 }
                            }
                        }
                    }
                }
            });

        });
    </script>

</body>
</html>
