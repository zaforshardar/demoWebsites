    ...
    <style>
        ...
        * {
        ...
        .filter-pills .btn.active {
            ...
            color: #fff;
            border-color: #1a5a7a;
            box-shadow: 0 4px 16px rgba(26, 90, 122, 0.25);
        }
        .filter-pills .btn i {
            margin-right: 6px;
        }
...
<body>
    ...
    <main class="container-fluid px-3 px-lg-4">
        ...
        <section class="row g-3 mb-3">
            <div class="col-12">
                <div class="d-flex flex-wrap align-items-center gap-2 filter-pills">
                    <span class="fw-semibold me-2"><i class="bi bi-funnel me-1"></i>Filter by disease:</span>
                    <button class="btn active" data-disease="all"><i class="bi bi-grid-3x3-gap-fill"></i> All</button>
                    <button class="btn" data-disease="dengue"><i class="bi bi-droplet"></i> Dengue</button>
                    <button class="btn" data-disease="malaria"><i class="bi bi-bug"></i> Malaria</button>
                    <button class="btn" data-disease="cholera"><i class="bi bi-droplet-half"></i> Cholera</button>
                    <button class="btn" data-disease="heatstroke"><i class="bi bi-thermometer-sun"></i> Heatstroke</button>
                    <span class="ms-auto text-muted small" id="filterStatus">Showing: WBGT (default)</span>
                </div>
            </div>
        </section>
        ...
        <section class="row g-3 mb-4" id="mapSection">
            <div class="col-12">
                <div class="map-wrapper">
                    <div id="ewarsMap"></div>
                    <!-- dynamic legend -->
                    <div class="position-absolute bottom-0 start-0 m-3 map-legend" id="mapLegend">
                        <span class="legend-title" id="legendTitle"><i class="bi bi-thermometer-half me-1"></i>WBGT Risk:</span>
        ...
        <section class="row g-3 mb-4" id="chartsSection">
            <div class="col-md-4">
                <div class="chart-card">
                    <h6 id="divChartTitle"><i class="bi bi-bar-chart-fill me-2"></i>Cases by Division</h6>
    ...
    <script>
        ...
        function getColorForDisease(value, maxVal) {
            if (maxVal === 0) return '#e9edf2';
            const intensity = Math.min(value / maxVal, 1);
            const r = 220 - intensity * 180;
            const g = 50 - intensity * 30;
            const b = 50 - intensity * 30;
            return `rgb(${Math.round(r)}, ${Math.max(Math.round(g), 20)}, ${Math.max(Math.round(b), 20)})`;
        }
        ...
        function renderTable(data) {
            ...
            tableInstance = $('#alertTable').DataTable({
                pageLength: 10,
                order: [
                    [6, 'desc']
                ],
        ...
        function initMap() {
            mapInstance = L.map('ewarsMap', { zoomControl: true }).setView([23.7, 90.35], 7);
            ...
            fetch('https://mobile.bmd.gov.bd/bmdmobile/district.min.json')
                ...
                .then(data => {
                    geoLayer = L.geoJSON(data, {
                        style: feature => getDistrictStyle(feature, 'all'),
                        onEachFeature: (feature, layer) => {
        ...
        function getDistrictStyle(feature, disease) {
            const name = feature.properties?.NAME || feature.properties?.name || '';
            const match = districtData.find(d => d.district.toLowerCase() === name.toLowerCase());
            if (!match) return { fillColor: '#e9edf2', fillOpacity: 0.3, color: '#94a3b8', weight: 0.8 };
            ...
            if (disease === 'all') {
                ...
                const val = match[disease] || 0;
                const maxVal = Math.max(...districtData.map(d => d[disease] || 0), 1);
                return {
                    fillColor: getColorForDisease(val, maxVal),
                    fillOpacity: 0.7,
                    color: '#1e293b',
                    weight: 1.0
                };
        ...
        function updateMap(disease) {
            ...
            geoLayer.eachLayer(layer => {
                ...
                if (feature) {
                    const style = getDistrictStyle(feature, disease);
            ...
            if (disease === 'all') {
                title.innerHTML = '<i class="bi bi-thermometer-half me-1"></i>WBGT Risk:';
                legend.innerHTML = `
                    <span class="legend-title" id="legendTitle"><i class="bi bi-thermometer-half me-1"></i>WBGT Risk:</span>
                    <span class="legend-item"><span class="dot" style="background:#0d6efd;"></span>&lt;21°C</span>
                    <span class="legend-item"><span class="dot" style="background:#0dcaf0;"></span>21–25</span>
                ...
                const diseaseLabels = {
                    ...
                    heatstroke: 'Heatstroke Cases'
                };
                title.innerHTML = `<i class="bi bi-droplet me-1"></i>${diseaseLabels[disease] || disease}:`;
                legend.innerHTML = `
                    <span class="legend-title" id="legendTitle"><i class="bi bi-droplet me-1"></i>${diseaseLabels[disease] || disease}:</span>
                    <span class="legend-item"><span class="dot" style="background:#fde5d9;"></span> Low</span>
                    <span class="legend-item"><span class="dot" style="background:#fc9272;"></span> Medium</span>
                    <span class="legend-item"><span class="dot" style="background:#cb181d;"></span> High</span>
                `;
            }
        }
        ...
        function updateCharts(disease) {
            // Update division bar chart
            const totals = {};
            districtData.forEach(d => {
                if (!totals[d.division]) totals[d.division] = 0;
                if (disease === 'all') {
                    totals[d.division] += d.dengue;
                } else {
                    totals[d.division] += d[disease] || 0;
                }
            });
            ...
            const chartTitle = disease === 'all' ? 'Dengue Cases by Division' :
                `${disease.charAt(0).toUpperCase() + disease.slice(1)} Cases by Division`;
            document.getElementById('divChartTitle').innerHTML = `<i class="bi bi-bar-chart-fill me-2"></i>${chartTitle}`;

            divChartInstance = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: labels,
                    datasets: [{
            ...
            // Update disease burden doughnut (always shows all diseases for comparison)
            const diseaseTotals = {
            ...
            // Re-initialize or update existing doughnut (simplify: just redraw)
            const dCtx = document.getElementById('diseaseChart').getContext('2d');
            // Destroy existing if any (using a global ref)
            if (window.diseaseChartInstance) window.diseaseChartInstance.destroy();
            window.diseaseChartInstance = new Chart(dCtx, {
                type: 'doughnut',
                data: {
        ...
        function filterByDisease(disease) {
            ...
            const names = {
                all: 'Showing: WBGT (default)',
                dengue: 'Showing: Dengue case burden',
                malaria: 'Showing: Malaria case burden',
                cholera: 'Showing: Cholera case burden',
                heatstroke: 'Showing: Heatstroke case burden'
            };
            status.textContent = names[disease] || '';

            // Filter table data
            let filteredData = districtData;
            if (disease !== 'all') {
                filteredData = districtData.filter(d => d[disease] > 0);
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Age chart (static, always dengue)
            ...
            console.log('✅ EWARS Demo with disease filters ready ·', districtData.length, 'districts loaded');
        });
    </script>
